...
# Post-installation script for Bostonclaw on Linux
...
# Create symbolic link for Bostonclaw app binary
if [ -x /opt/Bostonclaw/bostonclaw ]; then
    ln -sf /opt/Bostonclaw/bostonclaw /usr/local/bin/bostonclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/Bostonclaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/Bostonclaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/Bostonclaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/Bostonclaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/bostonclaw'
...
echo "Bostonclaw has been installed successfully."
