; Bostonclaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting Bostonclaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers Bostonclaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if Bostonclaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old Bostonclaw was installed
  ...
  ; "Can't modify Bostonclaw's files" retry loop -> "Bostonclaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "Bostonclaw 无法关闭" dialog.
...
; "Bostonclaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with Bostonclaw's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove Bostonclaw application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\bostonclaw (local app data)$\r$\n  • AppData\Roaming\bostonclaw (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering Bostonclaw processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\bostonclaw"
    RMDir /r "$APPDATA\bostonclaw"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\bostonclaw
    IfFileExists "$LOCALAPPDATA\bostonclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\bostonclaw"
      IfFileExists "$LOCALAPPDATA\bostonclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\bostonclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\bostonclaw
    IfFileExists "$APPDATA\bostonclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\bostonclaw"
      IfFileExists "$APPDATA\bostonclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\bostonclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\bostonclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\bostonclaw"
    IfFileExists "$APPDATA\bostonclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\bostonclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\bostonclaw"
    RMDir /r "$R3\AppData\Roaming\bostonclaw"
