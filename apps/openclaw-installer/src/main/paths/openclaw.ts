...
async function installOpenClaw(
  ...
  const openclawCmd = process.platform === 'win32'
    ...
    : join(npmPrefix, 'bin', 'openclaw');

  if (!existsSync(openclawCmd)) {
...
async function writeOpenClawConfig(): Promise<void> {
...
async function startGateway(
  ...
  const child = spawn(openclawCmd, args, {
    detached: true,
    // Windows .cmd requires shell execution.
    shell: isWin,
...
export async function run(plan: InstallPlan, onProgress: (p: InstallProgress) => void): Promise<RunResult> {
  ...
  try {
    ...
    const openclawCmd = await installOpenClaw(nodePath, onProgress);
    pushLog(`OpenClaw: ${openclawCmd}`);

    await writeOpenClawConfig();
    pushLog('配置已写入');

    await startGateway(openclawCmd, onProgress);
