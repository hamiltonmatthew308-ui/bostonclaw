...
interface SyncProxyOptions {
   ...
   * currently disabled in Bostonclaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync Bostonclaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because Bostonclaw proxy is disabled and preserve mode is enabled');
      return;
    }
