 ...
 * Used by Bostonclaw to seed the provider store when it's empty but providers are
...
/**
 * Write the Bostonclaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged Bostonclaw loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  Bostonclaw sets
 ...
 * OpenClaw/Bostonclaw versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When Bostonclaw-managed moonshot provider exists, prefer centralized key
    ...
    // for Bostonclaw to properly integrate with its updated tool system.
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // id="openclaw-qqbot" in its manifest, but older Bostonclaw versions
    ...
    // but Bostonclaw historically stored them only under `channels.<type>.accounts.default`.
