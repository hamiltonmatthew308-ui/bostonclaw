 ...
 * "openrouter.ai" carries the Bostonclaw app-attribution headers.
 *
...
(function () {
  ...
  if (globalThis.__bostonclawFetchPatched) return;
  globalThis.__bostonclawFetchPatched = true;

  globalThis.fetch = function bostonclawFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      delete flat['x-title'];
      delete flat['X-Title'];
      flat['HTTP-Referer'] = 'https://bostonclaw.app';
      flat['X-Title'] = 'Bostonclaw';
      init.headers = flat;
    }
