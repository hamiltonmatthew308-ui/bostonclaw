...
import { ensureBostonclawContext, repairBostonclawOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.bostonclaw.desktop';
...
// On Wayland this maps the running window to bostonclaw.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('bostonclaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[Bostonclaw] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[Bostonclaw] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[Bostonclaw] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== Bostonclaw Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain Bostonclaw markers (no OpenClaw
  // template content). This fixes a race condition where ensureBostonclawContext()
  // previously created the file before the gateway could seed the full template.
  if (!isE2EMode) {
    void repairBostonclawOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running' && !isE2EMode) {
      void ensureBostonclawContext().catch((error) => {
        logger.warn('Failed to re-merge Bostonclaw context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge Bostonclaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureBostonclawContext will retry until the target files appear.
  if (!isE2EMode) {
    void ensureBostonclawContext().catch((error) => {
      logger.warn('Failed to merge Bostonclaw context into workspace:', error);
    });
  }
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second Bostonclaw instance detected; redirecting to the existing window');
